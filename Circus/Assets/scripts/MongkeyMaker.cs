...
public class MongkeyMaker : MonoBehaviour
{
    ...
    public GameObject twoMonkey = null;
    ...
    void Start()
    {
        ...
        for(int i = 0; i< spawnner.Length; i++ ){
            if(i%5 ==  1){
                GameObject Objs = Instantiate(twoMonkey);
         ...
         if (!chk)
            {
                ...
                if (count == 4)
                {
                    ...
                    spawnner[pooingCount+1].SetActive(true);
                    spawnner[pooingCount+1].transform.position =new Vector3 (player.transform.position.x+14f,0.35f,0);
